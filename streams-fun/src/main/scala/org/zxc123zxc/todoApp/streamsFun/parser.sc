...
trait Parsers[Parser[+_]] {
  ...
  def size[A](p: Parser[A]): Parser[Int] = map(slice(p))(_.length)
  def product[A, B](pa: Parser[A], pb: => Parser[B]): Parser[(A, B)] = map2(pa, pb)((a, b) => (a, b))
  ...
  def skipL[A, B](pa: Parser[A], pb: Parser[B]): Parser[B] = product(pa, pb).map {case (_, r) => r}
  def skipR[A, B](pa: Parser[A], pb: Parser[B]): Parser[A] = product(pa, pb).map {case (l, _) => l}
  ...
  case class ParserOps[A](p: Parser[A]) {
    ...
    def pslice: Parser[String] = self.slice(p)
...
object Tmp {
  ...
  object SimpleParsers extends Parsers[Parser] {
    ...
    override def slice[A](p: Parser[A]): Parser[String] = s => {
      val offset = s.loc.offset
      val r = p(s)

      r match {
        case Succeed(_, l) =>
          val res = s.input.substring(offset, offset + l)
          Succeed(res, l)
        case f @ Failure(_) => f
      }
    }
    ...
    override def flatMap[A, B](pa: Parser[A])(f: A => Parser[B]): Parser[B] = s => {
      ...
      res match {
        case Succeed(r, l) =>
          ...
          val subState = s.copy(loc = loc)
          val subParser = f(r)
          val subRes = subParser(subState)

          subRes.advanceSuccess(l) // We need to add offset from previous result
